...
Design obstacle
In addition to the comments we made in the presentation, there is a major obstacle in our development. In our design, as the pop-up menu is loaded each time when users click it, a background process is running persistently to collect and intercept all the web requests. As our extension shows the pop menu based on each tab, it is required to get the urls that associate with certain tabs. Although it is not very hard to get these information from background process, there are many use cases we need to consider. For example, if the user refreshes the current tab, we should be able to reflect the change on origins in the pop-up menu. But in chrome, the navigation event does not has an consistent time stamp with web request. So it is possible that we get an new navigation event after receiving new web requests. We deal with this problem by detecting the domain change when intercepting web requests. However, there are still some corner cases that we haven’t considered. 
